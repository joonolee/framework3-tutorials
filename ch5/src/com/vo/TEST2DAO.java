...
*/
public class TEST2DAO extends AbstractOrmDao {
	// Update Only Map
	...
	public TEST2DAO(DB db) {
		super(db);
	}

	public String getInsertSql() {
		StringBuilder query = new StringBuilder();
		query.append("INSERT INTO TEST2 (");
	...
	public String getDeleteSql() {
		StringBuilder query = new StringBuilder();
		query.append("DELETE FROM TEST2 WHERE 1 = 1 ");
		query.append("	AND PK = ?");
	...
	public RecordSet select(ValueObject obj) {
		TEST2VO vo = (TEST2VO) obj;
		StringBuilder query = new StringBuilder();
		query.append("SELECT PK,COL1,COL2,ENTERDATE ");
		query.append("FROM TEST2 ");
		query.append("WHERE 1 = 1 ");
		query.append("	AND PK = ?");
	...
	public String getUserDeleteSql(String[] keys) {
		...
		StringBuilder query = new StringBuilder();
		query.append("DELETE FROM TEST2 ");
		query.append("WHERE 1 = 1 ");
		for (String key : keys) {
			query.append("	AND " + key + " = ?");
		}
